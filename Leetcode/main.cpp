...
bool pair_comparator(const Pair &p1, const Pair &p2){
...
public:
    vector<int> twoSum(vector<int> &numbers, int target) {
        int n = numbers.size();
	vector<Pair> orders(n);
	For(i,0,n){
	    orders[i].first = numbers[i];
	    orders[i].second = i;
	}
        sort(orders.begin(), orders.end(), pair_comparator);
	For(i,0,n)
	    numbers[i] = orders[i].first;
	
        for(int i =0 ; i < n; i++){
	    int rest = target-numbers[i];
	    vector<int>::iterator it;
	    if(rest >= numbers[i])
		it = lower_bound(numbers.begin()+i+1, numbers.end(),rest);
	    else
		it = lower_bound(numbers.begin(), numbers.begin()+i,rest);
            if(it!=numbers.end() && *(it)==rest){
		vector<int> result{orders[i].second+1,orders[(int)(it-numbers.begin())].second +1};
